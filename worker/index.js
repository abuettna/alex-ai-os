...
async function saveToAirtable(data, env) {
  ...
  const storedResponse = { responseId, submittedAt, ...data };
  const fields = {
...
async function handleDiscovery(request, env) {
  ...
  if (new TextEncoder().encode(rawBody).length > 25000) {
    return jsonResponse(413, { error: "Request too large" });
  }
  ...
  try {
    const responseId = await saveToAirtable(data, env);
    return jsonResponse(200, { saved: true, responseId });
  } catch (error) {
